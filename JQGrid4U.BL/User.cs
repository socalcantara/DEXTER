    ...
    public class UserBusinessLogic
	{
        ...
        public IEnumerable<UserSub> SiteList
        {
            get
            {
                ...
                using (SqlConnection conObj = new SqlConnection(conStr))
                {
                    SqlCommand cmdObj = new SqlCommand("select SiteID,Description as SiteName from Site", conObj);
                    conObj.Open();
                    SqlDataReader readerObj = cmdObj.ExecuteReader();
                    ...
                    while (readerObj.Read())
                    {
                        ...
                        SiteList.ID = Convert.ToInt32(readerObj["siteid"]);
		...
		public Boolean isAcctDisabled(string yUser)
		{
			Boolean vRet = false;
			using (SqlConnection conObj = new SqlConnection(conStr))
			{
				...
				SqlCommand cmdObj = new SqlCommand("sp_isacctdisabled", conObj);
                cmdObj.CommandType = CommandType.StoredProcedure;
                cmdObj.Parameters.Add(new SqlParameter("@username", yUser));
				conObj.Open();
				SqlDataReader readerObj = cmdObj.ExecuteReader();
